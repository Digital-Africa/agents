...
class PitchDesc:
    """
    A class to handle the pitch description process for startups.
    
    This class manages the retrieval, storage, and documentation of startup pitch decks
    ...
    Attributes:
        ...
        database (str): Notion database ID for pitch descriptions
        ...
        tags (list): List of tags for the pitch description
        id_file (str): ID of the pitch deck file
    ...
    def __init__(self, startup):
        ...
        self.logger.info("Initializing PitchDesc for startup", extra={"startup_id": self.data.get('id')})
        ...
        self.tags = ['pitch_desc']
        self.id_file = self.data['pitch_deck'].split('/')[-1]
        self.not_exists = self._not_exists()
        if self.not_exists:
            self.logger.info("No existing pitch description found, proceeding with creation")
            self.blob_meta = self.transform()
            self.match_startups_pool = self.match_satori_id(self.data['id'])[0]['id']
        else:
            self.logger.info("Existing pitch description found, skipping creation")
    ...
    def store_to_gcs(self):
        """
        Store the pitch deck file to Google Cloud Storage.
    ...
    def _not_exists(self):
        """
        Check if a pitch description record already exists in the Notion database.
        ...
        """
        self.logger.debug("Checking for existing pitch description", extra={"file_id": self.id_file})
        filter_ = {
            "property": "id_file",
            "rich_text": {
                "equals": self.id_file
    ...
    def transform(self):
        """
        Transform and store the pitch deck data.
        
        Handles the process of storing the pitch deck file to GCS and preparing
        its metadata for Notion integration.
        
        Returns:
            dict: Metadata about the stored file including URLs and content type
        """
        self.logger.info("Starting pitch deck transformation process")
        try:
    ...
    def build(self):
        """
        Build the Notion page structure for the pitch description.
        ...
        try:
            ...
            properties = dict()
            properties['Memo'] = self.writer.title(f"{self.data['name']} - pitch_desc")
            ...
            properties['Files & media'] = self.writer.embed_file(title=f"{self.data['name']} - pitch_desc", url=url)
            children = [self.childwriter.paragraph(f"This is the pitch desc {self.data['name']}"), self.childwriter.embed_file(url)]
    ...
    def run(self):
        """
        Execute the pitch description record creation process.
        ...
        """
        self.logger.info("Starting pitch description creation process")
        try:
            if self.not_exists:
                ...
                if response:
                    self.logger.info("Pitch description creation queued successfully")
                    return {"status": "success", "message": "Pitch description record queued for creation"}
                else:
                    self.logger.error("Failed to queue pitch description creation")
                    return {"status": "error", "message": "Failed to queue pitch description record"}
            else:
                self.logger.info("Pitch description already exists, skipping creation")
                return {"status": "info", "message": "Pitch description record already exists"}
                
        except Exception as e:
            self.logger.error("Pitch description creation process failed", extra={"error": str(e)})
