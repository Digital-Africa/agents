...
class Logo:
    """
    A class to handle the logo process for startups.
    
    This class manages the retrieval, storage, and documentation of startup logos
    ...
    Attributes:
        ...
        database (str): Notion database ID for logos
        ...
        tags (list): List of tags for the logo
        id_file (str): ID of the logo file
    ...
    def __init__(self, startup):
        ...
        self.logger.info("Initializing Logo for startup", extra={"startup_id": self.data.get('id')})
        ...
        self.tags = ['logo']
        self.id_file = self.data['logo'].split('/')[-1]
        self.not_exists = self._not_exists()
        if self.not_exists:
            self.logger.info("No existing logo found, proceeding with creation")
            self.blob_meta = self.transform()
            self.match_startups_pool = self.match_satori_id(self.data['id'])[0]['id']
        else:
            self.logger.info("Existing logo found, skipping creation")
    ...
    def store_to_gcs(self):
        """
        Store the logo file to Google Cloud Storage.
    ...
    def _not_exists(self):
        """
        Check if a logo record already exists in the Notion database.
        ...
        """
        self.logger.debug("Checking for existing logo", extra={"file_id": self.id_file})
        filter_ = {
            "property": "id_file",
            "rich_text": {
                "equals": self.id_file
    ...
    def transform(self):
        """
        Transform and store the logo data.
        
        Handles the process of storing the logo file to GCS and preparing
        its metadata for Notion integration.
        
        Returns:
            dict: Metadata about the stored file including URLs and content type
        """
        self.logger.info("Starting logo transformation process")
        try:
    ...
    def build(self):
        """
        Build the Notion page structure for the logo.
        ...
        try:
            ...
            properties = dict()
            properties['Memo'] = self.writer.title(f"{self.data['name']} - logo")
            ...
            properties['Files & media'] = self.writer.embed_file(title=f"{self.data['name']} - logo", url=url)
            children = [self.childwriter.paragraph(f"This is the logo {self.data['name']}"), self.childwriter.embed_file(url)]
    ...
    def run(self):
        """
        Execute the logo record creation process.
        ...
        """
        self.logger.info("Starting logo creation process")
        try:
            if self.not_exists:
                ...
                if response:
                    self.logger.info("logo creation queued successfully")
                    return {"status": "success", "message": "logo record queued for creation"}
                else:
                    self.logger.error("Failed to queue logo creation")
                    return {"status": "error", "message": "Failed to queue logo record"}
            else:
                self.logger.info("logo already exists, skipping creation")
                return {"status": "info", "message": "logo record already exists"}
                
        except Exception as e:
            self.logger.error("logo creation process failed", extra={"error": str(e)})
